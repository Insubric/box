...
object MapStyle {
  ...
  def simpleStyle(color:String = "rgba(237, 28, 36)", fillColor:String = "rgb(237, 28, 36,0.2)"):StyleFunction = (feature,resolution) => {
    if(resolution < 30 ) {
      ...
      new styleMod.Style(styleStyleMod.Options()
        .setFill(new styleMod.Fill(styleFillMod.Options().setColor(fillColor)))
        .setStroke(new styleMod.Stroke(styleStrokeMod.Options().setColor(color).setWidth(10)))
        .setImage(
          new styleMod.Circle(styleCircleMod.Options(3)
            .setFill(
              new styleMod.Fill(styleFillMod.Options().setColor(color))
