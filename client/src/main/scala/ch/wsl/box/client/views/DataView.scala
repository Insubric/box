...
case class DataView(model:ModelProperty[DataModel], presenter:DataPresenter) extends View {
  ...
  def loaded(metadata: JSONMetadata) = div(
    ...
    br,
    JSONMetadataRenderer(metadata, model.subProp(_.queryData),Seq(),Property(model.get.queryData.ID(metadata.keys).map(_.asString)),WidgetCallbackActions.noAction,Property(false)).edit(),
