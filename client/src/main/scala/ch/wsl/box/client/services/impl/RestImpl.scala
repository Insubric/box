...
import ch.wsl.box.model.shared.{BoxTranslationsFields, CSVTable, CurrentUser, DataResultTable, EntityKind, ExportDef, Field, GeoJson, IDs, JSONCount, JSONFieldMap, JSONID, JSONLookup, JSONLookups, JSONLookupsRequest, JSONMetadata, JSONQuery, LoginRequest, NewsEntry, PDFTable, TableAccess, XLSTable}
...
class RestImpl(httpClient:HttpClient) extends REST with Logging {
  ...
  override def geoData(kind:String, lang:String, entity:String, query:JSONQuery): Future[Map[String, Seq[GeoJson.Geometry]]] = {
    httpClient.post[JSONQuery,Map[String, Seq[GeoJson.Geometry]]](Routes.apiV1(s"/${EntityKind(kind).entityOrForm}/$lang/$entity/geo-data"),query)
  }
