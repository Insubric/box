...
import ch.wsl.box.model.shared.GeoJson.{CRS, Coordinates, FeatureCollection, Geometry, Point}
...
class MapListUpdateTest extends TestBase {
  ...
  val geomData:Geometry =  Point(Coordinates(x,y),CRS("EPSG:2056"))

  var _data:Json = Json.fromFields(Map(
    idField -> Json.fromInt(id),
    geomField -> geomData.asJson
  ))
  ...
  val _metadata = JSONMetadata.simple(UUID.randomUUID(),"form",formName,"it",Seq(
  ...
  class MockValues extends Values(loggerLevel){
    override def metadata: JSONMetadata = _metadata

    override def get(id: JSONID): Json = _data

    override def update(id: JSONID, data: Json): Json = {
  ...
  "Map widget" should "be insertable when geom in required" in test{
    for {
      ...
      _ <- waitLoggedIn
      _ <- Future {
        Context.applicationInstance.goTo(EntityFormState("form", formName, "true", JSONID.fromData(_data,_metadata).map(_.asString), public = false))
      }
      _ <- waitElement(() => document.querySelector(s".${TestHooks.mapControlButton(Control.POINT)}"),"Waiting insert point button")
      _ <- waiter(() => Try(TestHooks.map.getView().getResolution() != js.undefined).getOrElse(false))
      _ <- formLoaded()
