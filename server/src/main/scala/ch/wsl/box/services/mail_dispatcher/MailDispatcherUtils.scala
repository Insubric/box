  ...
  def listToSend():Future[Seq[(UUID,Mail)]] = fetchPending().flatMap{ rows => Future.sequence(rows.map{ boxMail =>
    boxMail.params match {
      case Some(params) => {
        ...
        } yield (boxMail.id.get, Mail(boxMail.mail_from,boxMail.mail_to,boxMail.mail_cc,boxMail.mail_bcc,subject,text,html))
      }
      case None => Future.successful{
        (boxMail.id.get, Mail(boxMail.mail_from,boxMail.mail_to,boxMail.mail_cc,boxMail.mail_bcc,boxMail.subject,boxMail.text,boxMail.html))
      }
    }
