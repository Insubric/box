...
object NewsUIDef {
  ...
  val main = JSONMetadata(
    ...
    layout = Layout(
      blocks = Seq(
        LayoutBlock.simple(4,Seq("datetime","author")),
        LayoutBlock(Some(Left("Translations")), Seq("news_i18n").map(Left(_)),8)
      )
    ),
//    layout = parse("""
//        |""").toOption.flatMap(_.as[Layout].toOption).getOrElse(Layout(Seq())),
  ...
  def newsI18n(langs:Seq[String]) = JSONMetadata(
    ...
    layout = Layout(
      blocks = Seq(
        LayoutBlock.simple(3, Seq("lang")),
        LayoutBlock.simple(9, Seq("title","text")),
      )
    ),
    entity = "news_i18n",
    lang = "en",
