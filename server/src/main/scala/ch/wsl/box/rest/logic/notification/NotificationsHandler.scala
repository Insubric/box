...
object NotificationsHandler {
  ...
  def create(channel:String,callback: (String) => Future[Boolean])(implicit ec:ExecutionContext):PgNotifier = new PgNotifier {
    val listener = new Listener(channel,callback)
    listener.start()
...
class Listener(channel:String,callback: (String) => Future[Boolean])(implicit ec:ExecutionContext) extends Thread with Logging {
