...
create function v_field_upd() returns trigger language plpgsql as $$
begin
    update field
        set type = new.type,
            name = new.name,
...
create or replace function v_field_ins() returns trigger language plpgsql as $$
begin
    insert into field (type, name, widget, "lookupEntity", "lookupValueField", "lookupQuery", "masterFields", "childFields", "childQuery", "default", "conditionFieldId", "conditionValues", params, read_only, required, form_uuid, child_form_uuid, function) values
...
create or replace function v_field_del() returns trigger language plpgsql as $$
begin
...
create trigger v_field_del instead of delete on v_field for each row execute function v_field_del();
create trigger v_field_ins instead of insert on v_field for each row execute function v_field_ins();
create trigger v_field_upd instead of update on v_field for each row execute function v_field_upd();