alter table form_actions add column execute_function text;
alter table form_actions add column action_order double precision;

create table form_navigation_actions
(
...
INSERT INTO form_actions (action, importance, after_action_goto, label, update_only, insert_only, reload, confirm_text, form_uuid, execute_function,action_order)
select 'SaveAction', 'Primary', null, 'form.save', true, false, true, null, form_uuid, null,1
from form
where form_uuid not in (select distinct form_uuid from form_actions) and entity <> 'box_static_page';

INSERT INTO form_actions (action, importance, after_action_goto, label, update_only, insert_only, reload, confirm_text, form_uuid, execute_function,action_order)
select 'SaveAction', 'Primary', '/box/$kind/$name/row/$writable/$id', 'form.save', false, true, false, null, form_uuid, null,2
from form
where form_uuid not in (select distinct form_uuid from form_actions where action_order is null) and entity <> 'box_static_page';
...
INSERT INTO form_navigation_actions (action, importance, after_action_goto, label, update_only, insert_only, reload, confirm_text, form_uuid, execute_function,action_order)
select 'NoAction', 'Std', '/box/$kind/$name', 'entity.table', false, false, false, null, form_uuid, null,1
from form
where entity <> 'box_static_page';


update form_actions set action_order=0 where action_order is null;

alter table form_actions alter column action_order set not null;

select * from form_navigation_actions
